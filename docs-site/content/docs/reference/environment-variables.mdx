...
| `MEMORY_LAYER_WRITER_ID` | Optional | Same as `--writer-id` / `--agent-id`: an advisory activity label. The authenticated principal remains authoritative. |
| `MEMORY_LAYER__SERVICE__API_TOKEN` | Usually generated | Service API token when supplied through env instead of TOML. |
| `MEMORY_LAYER_CLIENT_TOKEN` | Multi-user non-browser clients | Scoped `mlt_...` token used by CLI, TUI, watcher, and stdio MCP; takes precedence over the legacy service token. |
| `MEMORY_LAYER_OIDC_CLIENT_SECRET` | Multi-user browser login | Default environment variable containing the Authentik confidential-client secret. The name is configurable with `auth.oidc.client_secret_env`. |
...
MEMORY_LAYER_CLIENT_TOKEN='mlt_...' memory auth whoami
```
