...
| User-local `projects/<project-key>/config.toml` | Per-project runtime overrides and optional service/database/advisory writer settings. | No |
...
The default dev endpoint is `127.0.0.1:4250` for HTTP; live updates use `/ws`
on the same listener. The old `capnp_tcp_addr` and `capnp_unix_socket` settings
were removed in v2.

The adjacent project env file overrides shared env values. Define
`MEMORY_LAYER__SERVICE__API_TOKEN` there only when the project deliberately
uses a separate service; otherwise omit it so production commands keep the
shared service credential. If you do run a separate dev backend, make sure its
token and the dev client's token match.
