...
| `memory ingest` | Yes | Import documents from a directory, with or without a Git repository. |
...
| `memory consolidate` | Mixed | Find related memories and prepare higher-level insight proposals. |
| `memory structure` | No | Inspect the insight tree and discovered consolidation groups. |
| `memory review` | Mixed | Review validation corrections proposed for existing memories. |
...
```bash
memory capture --file task-context.json
...
Ingest is the document-first counterpart to repository scanning. It works when the source is a folder of notes, exported documents, or other material that does not need Git metadata.

```bash
memory ingest ./research --project memory --dry-run
memory ingest ./research --project memory
```
