...
| [Service and health](/docs/reference/cli/service-health) | `service`, `status`, `doctor`, `health`, `automation` | Starting the backend, diagnosing config, checking API health, and inspecting automation queues. |
...
| Service | `status`, `doctor`, `health` | `service enable`, `service disable`, `service restart-all`, automation flushes |
