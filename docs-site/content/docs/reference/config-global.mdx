...
| Section | Purpose |
| --- | --- |
| `[service]` | HTTP bind address, API token, request timeout, optional web root, and `read_only` (student mode: every mutating endpoint returns 403 while queries, briefings, and exports keep working — see the [classroom pack](/docs/classroom)). Live updates share the HTTP listener at `/ws`; v2 has no Cap'n Proto endpoint settings. |
| `[auth]`, `[auth.oidc]`, `[[auth.group_mappings.rules]]` | Single-user compatibility or Authentik multi-user mode, browser session lifetime, OIDC client, and global/project group roles. |
...
| `[writer]` | Optional advisory writer label for activity and diagnostics. The authenticated principal supplies durable authorship. |
...
For multi-user browser login and scoped workload credentials, read
[Authentication and access](/docs/reference/authentication). The legacy service
token is disabled in multi-user mode unless the temporary bootstrap flag is
explicitly enabled.
...
The `[procedural]` section controls [procedural utility learning](/docs/how-it-works/reinforcement#procedural-utility-loops-learn-their-worth): each automation loop learns a utility from how its proposals are received (ACT-R delta rule). Deterministic and advisory — it informs the loop listing and recommendations, never modes or permission gates — so it defaults on. The only runtime effect is the opt-in auto-trigger floor.
...
For prod/profile-installed commands, config is loaded from the explicit
`--config` path if supplied, otherwise from the discovered global config and
the current repo's user-local project config when available. Environment files
beside later project config can override shared values, and process variables
with the `MEMORY_LAYER__...` shape override TOML and env-file values.

Keep `MEMORY_LAYER__SERVICE__API_TOKEN` in the shared env file unless a project
really targets a separate service. If a project env file defines that key, its
token must authenticate the project-selected endpoint. `401` from repo-local
commands while `memory health` succeeds is a strong sign that endpoint and
token overrides do not belong to the same profile; compare the resolved paths
in `memory doctor --json` or pin the intended config with `--config`.
