...
| Section | Purpose |
| --- | --- |
| `[service]` | HTTP bind address, Cap'n Proto endpoints, API token, request timeout, optional web root, and `read_only` (student mode: every mutating endpoint returns 403 while queries, briefings, and exports keep working — see the [classroom pack](/docs/classroom)). |
