...
memory embeddings reindex --project <project-slug>
```
...
|---|---|---|
| Good exact query returns nothing | Wrong project slug or missing memory | `memory status --project <slug>` |
| Semantic search is empty | Embeddings missing or provider down | `memory embeddings list` |
| File-specific query misses context | Repo index or graph stale | `memory repo status --project <slug>`, `memory graph status --project <slug>` |
