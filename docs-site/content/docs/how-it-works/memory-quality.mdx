...
The mechanism is grounded in memory science — complementary learning systems and schema consolidation (Tse et al. 2007), sleep-replay gist extraction, the reflection trees of generative agents, and hierarchical summarization (RAPTOR, GraphRAG). It is off by default and dry-run first, like validation.

**Read more:** [`[consolidation]` config section](/docs/reference/config-global#consolidation-section) · [Automations](/docs/automations) · [`memory proposals`](/docs/reference/cli/capture-curation#memory-proposals)
