...
Utility is **advisory only**: `memory loops list --project <slug>` shows each loop's learned utility, decision count, and a recommendation ("consistently rejected; consider snoozing" / "high-value loop") once enough decisions accumulate — but it never changes a loop's mode or permission gates. The one opt-in runtime effect (`utility_floor_enabled`, default off) can suppress an *automatic* trigger for a loop whose utility has collapsed; manual runs are always unaffected. Deliberately unlike ACT-R, there is no noise term — utility is deterministic and clock-free, moved only by decisions.
