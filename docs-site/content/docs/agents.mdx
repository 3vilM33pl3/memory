...
For Codex Desktop, use the [Codex plugin](/docs/codex-plugin). It packages the supported stdio MCP connection and workflow skill; keep one Memory Layer MCP connection enabled so tools are not registered twice.
