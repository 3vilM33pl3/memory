...
The service can expose MCP over a local HTTP route, typically `/mcp`. Use this
only for local, token-bearing clients unless you add your own network and
authentication controls. In multi-user mode, HTTP MCP accepts a scoped service
token and deliberately ignores browser session cookies.
...
follow-up actions should happen.
```
...
- Create the token with `memory auth token create`; store it in the MCP client's secret configuration.
- Scope tools to explicit projects.
- Do not expose MCP HTTP to the public internet.

<Mermaid chart="sequenceDiagram\n    participant Client as MCP client\n    participant Server as Memory MCP server\n    participant API as Memory service API\n    Client->>Server: memory_search_all(question)\n    Server->>API: POST /v1/query/global\n    API-->>Server: answer, citations, project and repo metadata\n    Server-->>Client: structured MCP result" />

## Next

Read [Agents](/docs/agents), [Watchers](/docs/watchers), or [Operations](/docs/operations).
For Authentik, roles, and service-token setup, read [Authentication and access](/docs/reference/authentication).
