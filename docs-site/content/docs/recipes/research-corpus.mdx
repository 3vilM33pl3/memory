...
curl -s http://127.0.0.1:4040/v1/capture/task \
  -H "Authorization: Bearer $MEMORY_LAYER_CLIENT_TOKEN" -H "Content-Type: application/json" -d '{
...
curl -s http://127.0.0.1:4040/v1/curate \
  -H "Authorization: Bearer $MEMORY_LAYER_CLIENT_TOKEN" -H "Content-Type: application/json" \
  -d '{"project": "lit-review"}'
```

`writer_id` is a required advisory label for capture activity. The authenticated
principal remains the authoritative author in v2.
