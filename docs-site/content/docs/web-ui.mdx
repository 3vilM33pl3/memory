...
| Access | Global-admin inventory and controls for Authentik users, service tokens, and explicit project memberships. |
...
In multi-user mode, the top bar shows the Authentik identity and sign-out
control. The browser uses an HttpOnly session cookie plus CSRF protection and
never downloads a service token. Global admins see **Access** in the More menu;
new service-token secrets appear once and should be stored in OpenBao or another
secret manager before dismissal. See [Authentication and access](/docs/reference/authentication).
