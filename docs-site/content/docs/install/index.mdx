...
`memory wizard` runs the setup flow once, covering both the shared machine configuration (database URL, service URL and token, optional LLM/embedding providers — all skippable) and, when run inside a repository, the repo-local project setup (`.mem/project.toml` plus the `.agents/` directory with Memory Layer skills). Secrets are written outside your repository so they are never committed. Use `memory wizard --dry-run` to preview every file and service action first.
