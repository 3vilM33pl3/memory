...
isolated_prompt="$run_dir/prompt-isolated.md"
...
mkdir -p "$run_dir"
rm -f "$final_file" "$events_file" "$isolated_prompt"

{
  printf '%s\n\n' "Evaluation fixture constraints: do not run git commands; keep the app dependency-free; edit only index.html, styles.css, scripts/app.js, and scripts/check.sh inside the copied workspace; do not use external services except the harness-provided Memory helper when the prompt explicitly requires it."
  cat "$prompt_file"
} > "$isolated_prompt"
