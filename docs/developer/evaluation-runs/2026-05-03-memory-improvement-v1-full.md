...
Status: valid latest run. This report supersedes earlier same-day local
`memory-improvement-v1` artifacts from the 05:25, 05:26, and 05:30 batches,
which included non-benchmark agent-management overhead and must not be used for
token-cost or quality claims.
...
Do not mix this run with earlier same-day artifacts when comparing tokens or
success rates. The valid batch is identified by the completed artifacts listed
above and the comparison file
`target/memory-evals/memory-improvement-v1-comparison-20260503062007.json`.
