...
They are for the normal packaged setup, not for source-level development runs like `cargo run --bin memory --features embedded-service -- service run`, which activate a fully isolated dev profile on different ports. See [Dev Stack vs Installed Stack](../../developer/dev-stack.md) for the full contract.
