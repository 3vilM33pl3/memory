...
and [changelog](../../CHANGELOG.md) before updating a production installation.
...
The service applies migrations 0027-0032 when it starts. Do not downgrade the
binary against a migrated database; restore the pre-upgrade database backup if
you must roll back. Run `memory upgrade` only after reviewing the dry run
because it can refresh repo-local `.agents/` skills and instructions.
...
For v2.0.0, replace `<version>` with `2.0.0`. Every published package has a
