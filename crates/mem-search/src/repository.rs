...
#[cfg(test)]
pub(crate) async fn cleanup_test_project(pool: &PgPool, slug: &str) {
...
#[cfg(test)]
pub(crate) async fn insert_embedding_reindex_fixture(
    pool: &PgPool,
    slug: &str,
    project_id: Uuid,
    memory_id: Uuid,
) {
    cleanup_test_project(pool, slug).await;
...
#[cfg(test)]
pub(crate) async fn count_embeddings_for_space(
    pool: &PgPool,
    memory_id: Uuid,
    space_key: &str,
) -> i64 {
...
#[cfg(test)]
pub(crate) async fn insert_graph_query_fixture(
    pool: &PgPool,
    slug: &str,
    project_id: Uuid,
    memory_id: Uuid,
