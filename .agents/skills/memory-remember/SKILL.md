...
3. For direct no-plan actionable tasks, a `task` memory should already have been recorded at execution start; this workflow records the completed `implementation` or `refactor` memory.
4. Use `--type refactor` or clear refactor/no-functional-change wording when the completed work only reorganized code without behavior changes.
5. When `remember-task` is called with `--title` and `--prompt` for default, implementation, or refactor memory, the shared helper now runs idempotent `checkpoint start-task` first as a safety net. Use `--skip-task-start` only when the work is plan-backed, explanation-only, or the task start was intentionally handled elsewhere.
6. Provide one or more `--note` values for durable facts.
7. Only store verified outcomes and durable lessons.
8. For explanation memories, store a distilled reusable summary, not the whole chat answer.
9. Do not use `--file-changed` for explanation-only turns unless files actually changed.
10. If title, prompt, or summary are omitted, let the helper derive them from the repo state when possible.
